...
int main(int argc, char* argv[])
{
	...
	while(!done)
	{
		...
		if((my_ball.x_pos==target.x_pos)&&(my_ball.y_pos==target.y_pos))
		{
			gettimeofday(&tv2, NULL);
...
void set_up()
{
	...
	target.y_pos = 2;
	target.x_pos = 2;
	target.y_ttg=target.y_ttm=6;
	target.x_ttg=target.x_ttm=6;
	target.y_dir=1;
	target.x_dir=1;
	target.symbol = 't';
	...
	mvaddch(target.y_pos, target.x_pos, target.symbol);
...
void ball_move(int signum)
{
	...
	refresh_util(movedt, y_curt, x_curt, &target);
